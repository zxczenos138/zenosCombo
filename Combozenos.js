...
function AutoGapple()
  ...
  case "Matrix":
		    if(mc.thePlayer.motionX == 0.0 && mc.thePlayer.motionZ == 0.0) {
		    	  mc.thePlayer.motionY = 0.24; //0.05 worked before maybe i was glitched id                
        }
...
var entity = event.getTargetEntity();
		    if (entity instanceof EntityLivingBase && !(Timer.get() > 0 && !Delay.hasTimePassed(settings.Timer.get())) && (settings.Mode.get() == 'ComboHIT' || mc.thePlayer.onGround && !mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer, mc.thePlayer.getEntityBoundingBox().offset(0, -0.05, 0)).isEmpty()) && (entity.hurtTime == 0 || entity.hurtTime >= 4)) {
